...
class BalkaSpider(scrapy.Spider):
    name = "balka"
    allowed_domains = ["evraz.market"]
    start_urls = ["https://evraz.market/metalloprokat/fasonnyy_prokat/balka_dvutavrovaya/"]

    def start_requests(self):
        url = f"https://evraz.market/metalloprokat/fasonnyy_prokat/balka_dvutavrovaya/"
        yield SeleniumRequest(url=url, callback=self.parse_page)

    def parse_page(self, response: scrapy.http.Response):
       total_pages = 18 # find total pages number in first page
       for page in range(1, total_pages):
           url = 'https://evraz.market/metalloprokat/fasonnyy_prokat/balka_dvutavrovaya/?PAGEN_4=' + str(page)  # form page url 
